...
- Define all dependencies with their version explicitly in parent POM in its dependencyManagement section - without scope (except 'import')
- Define the required dependencies in child modules with scope and without version - order them semantically, first the dependencies that are required for the application, then the ones that are only required for testing. Within these groups insert new dependencies next to related dependencies (e.g. spring dependencies next to each other, persistence dependencies next to each other, etc.)

When working with Gradle projects:
- Use Gradle wrapper for consistent builds
- Prefer Kotlin DSL for build scripts
- Use version catalogs for dependency management

Prefere Maven over Gradle unless there is a compelling reason to use Gradle.
