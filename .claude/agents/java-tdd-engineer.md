...
- **Java 21 Idioms**:
    ...
    - **Stream API**: Use Streams for declarative, functional-style processing of collections.
      - **Prefer traditional for-loops and for-each loops**: Use traditional for-loops and for-each loops over stream-for-loops except for simple single invocations.
    - **`try-with-resources`**: Ensure automatic and safe resource management for all `AutoCloseable` resources.
- **Prefer Input/Output Streams over Files**: Use `InputStream` and `OutputStream` in service methods instead of `File` to enhance testability (no need to rely tests on the file system).
...
- **Capitalize only the first letter in CamelCase acronyms** - For acronyms in CamelCase, capitalize only the acronyms first letter: "TddSupportClass", "myTddSupport", "userId"
- **Never use two capital letters directly after each other in Names with mixed capitals qualifier names.** - Examples: Use "Id" instead of "ID"
...
**Nullability (JSpecify + NullAway)**:
- **Non-Null by default (`@NullMarked`)**: Apply `@org.jspecify.annotations.NullMarked` at **package level** (via `package-info.java`) for all production packages. This makes parameters, returns, and fields **non-null by default**. Use `@NullUnmarked` only locally as a temporary bridge in legacy edges.
- **Optionality Policy (per API/Module)**:
  - Choose **either** `Optional<T>` **or** `@org.jspecify.annotations.Nullable T` for optional **return values** and use it **consistently within the same API layer**.
  - **Parameters**: never use `Optional` as a parameter type. If optional, annotate with `@Nullable` and document how `null` is handled.
  - **Collections**: never return `null`; prefer **empty, immutable** collections. Never use `Optional<Collection<…>>`.
- **Contracts & JavaDoc (EN)**:
  - Document any `@Nullable` parameter/return explicitly and state how `null` is treated (ignored, defaulted, exception).
  - For non-null returns under `@NullMarked`, state "never returns `null`; may be empty" for collections.
- **Edges (DB/JSON/HTTP/Maps)**:
  - Treat all inbound data as **potentially null**; validate early with guards (e.g., `Objects.requireNonNull`, blank checks) and **convert** into non-null domain objects (empty collections instead of `null`, meaningful defaults, or clear exceptions).
  - Do not leak vendor-specific nullability into public APIs; keep adapters/wrappers at the boundary.
- **Tooling Setup (short)**:
  - Add `org.jspecify:jspecify` (compileOnly) and enable **Error Prone + NullAway** with `-Xep:NullAway:ERROR` and `-XepOpt:NullAway:AnnotatedPackages=<your root packages>`.
  - Align **IDE build** with CI (build via Gradle/Maven). CI must **fail on NullAway errors**.
- **Lombok Interop**:
  - Generated code may not carry JSpecify semantics; if needed, configure NullAway for generated sources (e.g., treat generated as unannotated) and keep **defensive copies** (`List.copyOf`) in builders/getters.
  - `@NonNull` from Lombok is **not** a substitute for `@NullMarked`; prefer JSpecify for semantics, Lombok for boilerplate.
- **Primitives & Boxing**:
  - Prefer primitives; use boxed types only when nullability is required (and then document it).
- **TDD & Tests for Nullability**:
  - Follow existing testing rules: **do not** write tests for simple parameter null-checks unless it is part of the business logic.
  - Write tests for **API contracts** that matter to consumers: returns **empty** collections (not `null`), `Optional.empty()` vs. present, or documented defaults/exceptions for optional inputs.
...
- No tests for checking parameter constraints, especially not if the check is just to protect from programming failures like nullchecks (unless they are part of the business logic)
- Use JUnit5 features: parameterized tests for data-driven scenarios
- Try not to use fields in test classes for values that are used in (multiple) tests, but use local variables instead. This helps to keep tests independent and avoids state leakage between tests.
...
- **Test functionality not getter and setter** Test code that can fail, but not getter and setter.

**Documentation**
- Document the (domain) model classes
  - Write Javadoc for classes and fields (private and public)
- Document the purpuse and scope of service classes.
  - Typicaly Do not document the injected fields of a service class, except there exists more than one instance of the injected type in the project.
...
- The service layer must keep free of any web related classes (like MultipartFile, HttpRequest, HttpResponse, HttpSession), these must be converted in the controller layer.
- In throw clause decladed exceptions must be specific, do not use generic java.lang.Exception (IOException is ok).
- Catch only runtim
